# CLAUDE.md — 番茄小说生成智能体 V2

## 项目概述

番茄小说生成智能体 V2（Tomato Novel Generator V2）—— 一句话输入，AI 自动生成完整小说。
支持风格多选、六层大纲、CrewAI 多智能体管线、XMind 导出、对话式创作、段落润色（重写/扩写/精简）。

> **版本标识**: 本目录为 V2 血肉版，API 前缀 `/api/v2`，数据库文件 `novel_generator_v2.db`
> V2 在 V1 基础上新增：段落润色、版本历史、ParagraphVersion 模型、生成记录管理增强
...
- **模型**: 多 Provider 工厂模式（OpenAI / Anthropic / Ollama / OpenCode Zen / MiMo V2.5 / 国产模型）
...
- `data.py` — 番茄分类/风格/国产模型数据（含 MiMo-V2.5）
...
2. 前端调用 `POST /api/v2/records/{id}/cancel` → 后端标记 `cancelled`
