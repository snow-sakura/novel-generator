...
function OutlineLayerCard({ type, data }) {
  ...
  if (type === 'structure') {
    ...
    rows = chs.slice(0, 8).map((ch, i) => ({
      ...
      val: ch.title || '',
      sub: [
        ch.summary ? `概要: ${ch.summary.slice(0, 80)}` : '',
        ch.hook ? `钩子: ${ch.hook.slice(0, 40)}` : '',
        ch.cliffhanger ? `悬念: ${ch.cliffhanger.slice(0, 40)}` : '',
      ].filter(Boolean).join(' · '),
    }))
...
function OutlineButtons({ outlineThinking, layersCount, showOutlineModal, showChaptersModal }) {
  const layers = outlineThinking.filter(item => item?.type && item.type !== '_progress' && item.type !== 'chapter')
  const hasChapters = useNovelStore(s => s.chapters.length > 0)
...
export default function MultiStepLog() {
  const { thinkingLogs, currentStep, generating, connecting, outlineThinking, chapters, chapterTexts } = useNovelStore()
  ...
  const layersCount = outlineThinking.filter(item => item?.type && item.type !== '_progress' && item.type !== 'chapter').length
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden animate-fade-in-up shadow-sm">
                    ...
                    <span className={cn('text-xs block mt-0.5',
                      ...
                      {isActive && key === 'writing'
                        ...
                        : isActive && key === 'outlining'
                          ? `已生成 ${outlineThinking.filter(o => o.type && o.type !== '_progress' && o.type !== 'chapter').length}/5 层`
                          : step.desc}
                    </span>
                  </div>
                  {hasLogs && !isOpen && (
                  ...
                  <div className="px-5 pb-4 animate-fade-in-down">
                    {key === 'outlining' && (
                      <OutlineButtons
                        ...
                        showChaptersModal={() => setShowChaptersModal(true)}
                      />
                    )}
      ...
      {/* 大纲弹窗 */}
      {showOutlineModal && (
        <OutlineModal outlineThinking={outlineThinking} onClose={() => setShowOutlineModal(false)} />
      )}
