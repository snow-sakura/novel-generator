...
- **TTS语音合成**：集成edge-tts多声优，支持单章和全本生成
- **AI配图功能**：生成/展示/删除封面配图，支持多种风格
- **统计分析**：词频统计、角色出现分析、情感曲线SVG可视化
- **段落润色**：重写/扩写/精简功能，支持版本历史对比
- **生成记录管理增强**：支持分页/轮询/继续/取消，状态追踪更完善

### 优化改进

- **前端UI优化**：多版本开头对比、对话插入确认、章节目录改进
- **后端性能优化**：SSE事件流优化、超时保护增强、数据库迁移改进
- **模型配置优化**：支持更多国产模型、分级策略更智能

### 文件变更

| 文件 | 说明 |
|---|---|
| `v3/frontend/src/pages/NovelPage.jsx` | 新增统计分析、配图管理功能 |
| `v3/frontend/src/components/TTSPlayer.jsx` | 新增TTS语音播放器组件 |
| `v3/backend/app/routers/tts.py` | 新增TTS语音合成API |
| `v3/backend/app/routers/illustrations.py` | 新增AI配图API |
| `v3/backend/app/routers/analysis.py` | 新增统计分析API |
