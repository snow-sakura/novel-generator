...
class Novel(Base):
    ...
    theme = Column(String(50), default="")               # 核心主题
    emotion_curve = Column(Text, default="")              # 情感曲线 JSON
    aesthetic_intensity = Column(String(10), default="中度")  # 美学强度：关闭/轻度/中度/重度
    interpretation = Column(Text, default="")                 # 文末解读（F5 意义提炼）
    character_bible = Column(Text, default="{}")              # 设定档案 JSON（F6 跨章节一致性）
    illustrations = Column(Text, default="[]")                # AI 配图 JSON（F11 [{chapter_index, prompt, url, generated_at}]）
