...
class GeneratorService:
    ...
    async def generate(
        ...
        try:
            ...
            outline_prompts = {
                "strategy": (custom_prompts or {}).get("outline_l1")
                or SYSTEM_PROMPT_L1_STRATEGY,
                "characters": (custom_prompts or {}).get("outline_l2")
                or SYSTEM_PROMPT_L2_CHARACTERS,
                "world": (custom_prompts or {}).get("outline_l3")
                or SYSTEM_PROMPT_L3_WORLD,
                "structure": (custom_prompts or {}).get("outline_l4")
                or SYSTEM_PROMPT_L4_STRUCTURE,
                "chapters": (custom_prompts or {}).get("outline_l5")
                or SYSTEM_PROMPT_L5_CHAPTERS,
            }
            chapter_prompt_tpl = (custom_prompts or {}).get(
                "chapter"
            ) or SYSTEM_PROMPT_CHAPTER
            ...
            if start_from_chapter == 0:
                ...
                story_elements = await parse_elements(
                    self.llm, seed_text, gender, genre, style, formatted_parse
                )
                ...
                full_outline = continuation.get(
                    "outline", {"chapters": chapters, "elements": story_elements}
                )
                outline_data = full_outline
                chapter_count = len(chapters)
                yield self._make_log(
                    f"📐 继续生成：已有 {start_from_chapter}/{chapter_count} 章，跳过解析和大纲"
                )
                if isinstance(outline_data, dict):
                    ...
                    for k in ("plot_structure", "rhythm", "style_tone"):
                        ...
                        if v and isinstance(v, dict):
                            _structure_data[k] = v
                    if _structure_data:
                        _outline_layers.append(
                            {"type": "structure", "data": _structure_data}
                        )
            ...
            # F6: 从大纲种子设定档案
            character_bible = seed_bible_from_outline(
                outline_data, story_elements, gender, genre, style
            )
            if character_bible:
                _log(
                    f"📖 初始设定档案就绪：{len(character_bible.get('characters', []))} 角色, {len(character_bible.get('locations', []))} 地点"
                )
            ...
            if not novel_id:
                ...
                update_novel_outline(
                    novel_id, chapters, story_elements, tree_fn=_dict_to_tree, db=_db
                )
            ...
            per_chapter_target = (
                word_count // len(chapters) if len(chapters) > 0 else 2000
            )
            per_chapter_target = max(
                per_chapter_min, min(per_chapter_max, per_chapter_target)
            )
            ...
            save_outline_mindmap(
                novel_folder,
                f"{'生成中' if start_from_chapter == 0 else '继续生成'}",
                chapters,
                story_elements,
                gender,
                genre,
            ...
            for i, chapter in enumerate(chapters):
                ...
                if outline_data:
                    ...
                    seed_reference = outline_data.get("elements", {}).get(
                        "inciting_incident", ""
                    )
                    if seed_reference:
                ...
                if enable_suspense:
                    tension_lines.append(
                        "- 悬念：已启用。每章结尾必须留有悬念/钩子（Cliffhanger），让读者忍不住点击下一章"
                    )
                if enable_twist:
                    tension_lines.append(
                        "- 反转：已启用。故事中期或结尾需安排至少一次意外反转，全篇不超过2次"
                    )
                tension_block = (
                    "\n【叙事张力】\n" + "\n".join(tension_lines) + "\n"
                    if tension_lines
                ...
                # F4: 美学风格块
                aesthetic_block = AESTHETIC_BLOCKS.get(
                    aesthetic_intensity, AESTHETIC_BLOCKS["中度"]
                )
                ...
                if opening_text and i == 0:
                    ...
                    previous_summary = (
                        f"上一章《{title}》概要：{chapter_content[:200]}..."
                    )
                    ...
                    save_single_chapter_file(
                        novel_folder, title, 0, chapter_content, chapters
                    )
                    if record_id:
                        update_record_progress(
                            record_id,
                    ...
                    for si, scene_desc in enumerate(scenes):
                        ...
                        for _attempt in range(2):
                            try:
                                ...
                                _log(
                                    f"⚠️ 第{i + 1}章·场景{si + 1}异常: {e}（第{_attempt + 1}次）"
                                )
                            if _attempt == 0:
                ...
                while actual_words < per_chapter_min and wc_retry < max_wc_retries:
                    ...
                    yield self._make_log(
                        f"  📏 第{i + 1}章仅{actual_words}字，不足{per_chapter_min}字，补写约{needed}字..."
                    )
                    continue_prompt = (
                    ...
                    for _attempt in range(2):
                        try:
                            ...
                            _log(
                                f"⚠️ 第{i + 1}章字数补全异常: {e}（第{_attempt + 1}次）"
                            )
                        if _attempt == 0:
                            await asyncio.sleep(1)
                    if extra.strip():
                ...
                if actual_words < per_chapter_min:
                    _log(
                        f"⚠️ 第{i + 1}章强制补全后仍不足{per_chapter_min}字（{actual_words}字）"
                    )
                ...
                if actual_words > cap:
                    _log(
                        f"⚠️ 第{i + 1}章字数{actual_words}超过上限{per_chapter_max}（上限120%={cap}），摘尾至{per_chapter_max}字"
                    )
                    yield self._make_log(
                        f"  ⚠️ 第{i + 1}章超出字数上限，自动裁减至{per_chapter_max}字"
                    )
                    ...
                    _log(
                        f"⚠️ 第{i + 1}章字数{actual_words}略超上限{per_chapter_max}（未超过120%）"
                    )
                ...
                # F6: 从本章提取设定档案更新
                try:
                    update = await extract_bible_update(
                        self.llm, character_bible, chapter_content, i + 1
                    )
                    if update:
            ...
            final_title = await generate_title(
                self.llm,
                full_content,
                gender,
                genre,
            ...
            if record_id:
                update_record_complete(
                    record_id, novel_id, chapter_states=chapter_states, db=_db
                )
            ...
            if record_id:
                ...
                update_record_error(
                    record_id, str(e), self._current_step, chapter_states=cs, db=_db
                )
            if novel_id:
    ...
    async def generate_dialogue(self, characters: list, scenario: str) -> str:
        ...
        for c in characters:
            ...
            if c.get("role"):
                parts.append(f"角色定位：{c['role']}")
            if c.get("traits"):
                parts.append(f"性格特征：{c['traits']}")
            if c.get("description"):
                parts.append(f"描述：{c['description']}")
            if c.get("relationships"):
                parts.append(f"关系：{c['relationships']}")
            if c.get("arc"):
                parts.append(f"成长弧光：{c['arc']}")
            profiles.append("\n".join(parts))
        char_str = "\n\n---\n\n".join(profiles)
        prompt = DIALOGUE_PROMPT.replace("{character_profiles}", char_str).replace(
            "{scenario}", scenario
        )
    ...
    async def generate_continuation(
        self, context: str, instruction: str = "", target_words: int = 300
        ...
        async for chunk in timeout_iterate(
            self.llm.generate_stream(prompt), timeout=60, first_chunk_timeout=30
        ):
            yield chunk
