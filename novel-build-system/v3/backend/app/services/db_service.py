...
def update_novel_content(novel_id: int, content: str, chapters: list,
    ...
    try:
        ...
        for idx, block in enumerate(blocks):
            ...
            existing = _db.query(ChapterContent).filter(
...
def update_novel_outline(novel_id: int, chapters: list, elements: dict,
                          tree_fn=None, db: Optional[Session] = None):
    """更新小说大纲数据"""
    _db, sc = _resolve(db)
    try:
        novel = _db.query(Novel).filter(Novel.id == novel_id).first()
...
def finalize_novel(novel_id, title, content, chapters, elements,
    ...
    try:
        novel = _db.query(Novel).filter(Novel.id == novel_id).first()
        if novel:
            db_chapters = _db.query(ChapterContent).filter(
                ChapterContent.novel_id == novel_id
...
def update_novel_index(title, seed_text, gender, genre, style,
...
def update_record_progress(record_id, completed, total, content,
                            chapter_states=None, db: Optional[Session] = None):
    """更新生成记录进度"""
    _db, sc = _resolve(db)
    try:
        rec = _db.query(GenerationRecord).filter(GenerationRecord.id == record_id).first()
        if rec:
...
def update_record_error(record_id, error, failed_step=None,
                         chapter_states=None, db: Optional[Session] = None):
    """标记生成记录为失败"""
    _db, sc = _resolve(db)
    try:
...
def update_record_complete(record_id, novel_id, chapter_states=None,
                            db: Optional[Session] = None):
    """标记生成记录为完成"""
    _db, sc = _resolve(db)
    try:
