...
async def export_novel(
    ...
    if format == "markdown":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename*=UTF-8''{_safe_filename(title, 'epub')}"
            },
        )
    else:
...
async def export_outline(
    ...
    if isinstance(outline, dict) and "_tree" not in outline:
        try:
            ...
            logger.warning(
                "export_outline: 生成大纲思维导图失败 (novel_id=%d): %s", novel_id, e
            )
...
async def export_package(novel_id: int, db: Session = Depends(get_db)):
    ...
    if isinstance(outline, dict) and "_tree" not in outline:
        try:
            ...
            logger.warning(
                "export_package: 生成大纲思维导图失败 (novel_id=%d): %s", novel_id, e
            )
    chapters = outline.get("chapters", [])
    if not chapters and novel.chapters:
        try:
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        try:
            ...
            logger.warning(
                "export_package: 生成 XMind 大纲失败 (novel_id=%d): %s", novel_id, e
            )
