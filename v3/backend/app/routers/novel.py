...
async def insert_paragraph(req: InsertParagraphRequest, db: Session = Depends(get_db)):
    ...
    insert_at = len(paragraphs) if req.paragraph_index == -1 else min(req.paragraph_index + 1, len(paragraphs))
...
async def get_novel(novel_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "bible": json.loads(novel.character_bible) if novel.character_bible and novel.character_bible != "{}" else {},
...
async def update_emotion_curve(novel_id: int, body: dict, db: Session = Depends(get_db)):
    novel = db.query(Novel).filter(Novel.id == novel_id).first()
    if not novel:
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def update_character_bible(novel_id: int, body: dict, db: Session = Depends(get_db)):
    novel = db.query(Novel).filter(Novel.id == novel_id).first()
    if not novel:
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def search_novel_content(novel_id: int, q: str = Query(...), db: Session = Depends(get_db)):
    """FTS5 全文搜索"""
    novel = db.query(Novel).filter(Novel.id == novel_id).first()
    if not novel:
        raise HTTPException(status_code=404, detail="小说不存在")
    results = db.execute(text("""
        ...
        ORDER BY rank
        LIMIT 20
    """), {"q": q, "novel_id": novel_id}).fetchall()
    return [
        {"chapter_index": r[1], "title": r[2], "snippet": r[3]}
        for r in results
    ]
