    ...
    try:
        ...
        if "novels" in tables:
            ...
            if "theme" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN theme VARCHAR(50) DEFAULT ''"))
                added.append("theme")
            if "emotion_curve" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN emotion_curve TEXT DEFAULT ''"))
                added.append("emotion_curve")
            if "aesthetic_intensity" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN aesthetic_intensity VARCHAR(10) DEFAULT '中度'"))
                added.append("aesthetic_intensity")
            if "interpretation" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN interpretation TEXT DEFAULT ''"))
                added.append("interpretation")
            if "character_bible" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN character_bible TEXT DEFAULT '{}'"))
                added.append("character_bible")
            if "illustrations" not in columns:
                db.execute(text("ALTER TABLE novels ADD COLUMN illustrations TEXT DEFAULT '[]'"))
        ...
        if "generation_records" in tables:
            ...
            if "outline_data" not in rec_columns:
                db.execute(text("ALTER TABLE generation_records ADD COLUMN outline_data TEXT DEFAULT '{}'"))
        ...
        if "chapter_contents" not in tables:
            ...
            for novel in db.query(Novel).filter(Novel.content != "", Novel.content.isnot(None)).all():
                blocks = [b.strip() for b in re.split(r"\n(?=## )", novel.content) if b and b.strip()]
        ...
        # FTS5 虚拟表（全文搜索）
        all_tables = [r[0] for r in db.execute(text("SELECT name FROM sqlite_master WHERE type='table'")).fetchall()]
        if "chapter_contents_fts" not in all_tables:
            ...
            """))
            db.execute(text("INSERT INTO chapter_contents_fts(chapter_contents_fts) VALUES('rebuild')"))
            db.commit()
