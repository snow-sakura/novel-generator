    ...
    const onEvent = (event, data) => {
      switch (event) {
        ...
        case 'chapter_end': {
          ...
          const chapters = (state.novelData?.chapters ?? []) as Array<{ title: string }>
          const chapterTexts = (state.novelData?.chapterTexts ?? []) as string[]
          const idx = chapters.findIndex(c => c.title === title)
          if (idx >= 0) {
            const texts = [...chapterTexts]
            texts[idx] = '[completed]'
            updateNovelData({ chapterTexts: texts })
          }
          const chapterNum = idx >= 0 ? idx + 1 : chapters.length + 1
          updateLastMessage(
