...
export interface BibleWorldRule {
  name?: string
  description?: string
  rule?: string
}

export interface BibleKeyItem {
  name: string
  description: string
}
...
export interface Bible {
  ...
  world_rules?: BibleWorldRule[]
  key_items?: BibleKeyItem[]
...
export interface OutlineThinkingItem {
  index: number
  title: string
  summary: string
}
...
export const SAMPLE_NOVEL: SampleNovel = {
  id: 0, title: '星穹之约',
...
export async function* mockGenerateOpenings(): AsyncGenerator<SSEMessage> {
...
export async function* mockGenerateStream(): AsyncGenerator<SSEMessage> {
  ...
  for (let i = 0; i < 4; i++) {
    const ch = JSON.parse(SAMPLE_NOVEL.chapters)[i] as OutlineChapter
...
export async function* mockGenerateDialogue(): AsyncGenerator<SSEMessage> {
