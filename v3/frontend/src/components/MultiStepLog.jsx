...
export default function MultiStepLog() {
  ...
  const layersCount = outlineThinking
    ...
    .flatMap(item => item.type === 'structure' && item.data
                    ...
                    <span className={cn('text-xs block mt-0.5',
                      ...
                      {isActive && key === 'writing'
                        ...
                        : isActive && key === 'outlining'
                          ? `已生成 ${outlineThinking.filter(o => o.type && o.type !== '_progress' && o.type !== 'chapter').flatMap(o => o.type === 'structure' && o.data ? ['plot_structure', 'rhythm', 'style_tone'].filter(k => o.data[k]) : [o]).length}/5 层`
                          : step.desc}
                    </span>
                  </div>
                  {hasLogs && !isOpen && (
