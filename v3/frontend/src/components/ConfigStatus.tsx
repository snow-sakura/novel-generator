...
export default function ConfigStatus() {
  ...
  async function doCheck() {
    ...
    setConfigStatus(true, info.configured, info as unknown as Record<string, unknown>)
    setChecking(false)
  }
  ...
  return (
    ...
    >
      {configOk ? (
        <>
          <CheckCircle className="w-3.5 h-3.5 flex-shrink-0" />
          <span>{configInfo.provider as string}</span>
          {configInfo.model && (
            <span className="text-gray-400 ml-1">({configInfo.model as string})</span>
          )}
        </>
      ) : (
        <>
          <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
          <span className="truncate max-w-[240px]">{configInfo.error as string}</span>
          <button
            onClick={doCheck}
            disabled={generating}
