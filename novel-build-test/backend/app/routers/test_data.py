...
async def list_data_sources(
    ...
    query = base_query.order_by(DataSource.updated_at.desc()).offset((page - 1) * page_size).limit(page_size)
...
async def list_masking_rules(
    ...
    query = select(MaskingRule).order_by(MaskingRule.name).offset((page - 1) * page_size).limit(page_size)
    result = await db.execute(query)
    items = list(result.scalars().all())
...
@router.put("/masking-rules/{rule_id}", response_model=MaskingRuleResponse)
...
@router.delete("/masking-rules/{rule_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def list_mock_services(
    ...
    query = base_query.order_by(MockService.updated_at.desc()).offset((page - 1) * page_size).limit(page_size)
    result = await db.execute(query)
    items = list(result.scalars().all())
...
async def update_mock_service(
    ...
    """更新 Mock 服务"""
    result = await db.execute(select(MockService).where(MockService.id == service_id))
...
@router.delete("/mocks/{service_id}", status_code=status.HTTP_204_NO_CONTENT)
