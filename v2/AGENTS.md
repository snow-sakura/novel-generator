...
- `POST /api/v2/cleanup` — 一键清理孤立的 in_progress 记录、"生成中..."小说、无主完成记录
- `POST /api/v2/records/{id}/reset` — 将卡住的 in_progress 记录重置为 failed
...
| POST `/api/v2/generate/continue?record_id=X` | 从失败/取消点继续 |
| POST `/api/v2/chat/generate` | AI 对话式生成 SSE |
| GET `/api/v2/records` | 生成记录（分页） |
| GET `/api/v2/records/{id}` | 记录详情 |
| GET `/api/v2/records/{id}/status` | 轻量状态轮询（用于 HistoryPage polling） |
| POST `/api/v2/records/{id}/cancel` | 标记记录为 cancelled（停止生成前调用） |
| POST `/api/v2/records/{id}/reset` | 重置卡住的 in_progress 记录为 failed |
| DELETE `/api/v2/records/{id}` | 删除记录 |
| POST `/api/v2/cleanup` | 清理孤立数据（无主记录 + 无效小说） |
| GET `/api/v2/novels/{id}/export?format=md/txt/pdf` | 全文导出 |
| GET `/api/v2/novels/{id}/export/chapters` | 逐章 ZIP |
| GET `/api/v2/novels/{id}/export/outline?format=markdown/xmind` | 大纲导出 |
| GET `/api/v2/models/list` | 国产模型列表 |
| GET `/api/v2/genres/list?gender=` | 题材列表 |
| POST `/api/v2/refine` | SSE 段落润色（重写/扩写/精简） |
| GET `/api/v2/refine/versions` | 获取段落版本历史 |
